...
def train_with_repeated_kfold_and_save(texts, labels):
    ...
    for fold, (train_idx, val_idx) in enumerate(rskf.split(np.zeros(len(labels)), labels_flat)):
        ...
        output_dir = f"/content/drive/MyDrive/portuguese_subnarrative_model/results_fold_{fold}"
        logging_dir = f"/content/drive/MyDrive/portuguese_subnarrative_model/logs_fold_{fold}"
    ...
    # Save final metrics
    with open("/content/drive/MyDrive/portuguese_subnarrative_model/final_metrics.json", "w") as f:
        json.dump({"mean_f1": mean_f1, "fold_f1_scores": all_f1_scores}, f, indent=4)

    # Save the final best model
    final_output_dir = "/content/drive/MyDrive/portuguese_subnarrative_model"
